...
const EditProfile = () => {
  ...
  const handleCountryChange = (selectedCountry: string) => {
    ...
    localStorage.setItem('selectedCountry', selectedCountry); // Зберігаємо вибрану країну
  
    let newLanguage: 'uk' | 'en' | 'cz' | 'de' = 'uk'; // Мова за замовчуванням
    let newRegion = ''; // Регіон за замовчуванням
  
    // Заміна languages на language
    if (['США', 'Канада', 'Мексика'].includes(selectedCountry)) {
      newLanguage = 'en';
      newRegion = language.northAmerica; // Регіон North America
      ...
      newRegion = language.europe; // Регіон Europe
      console.log("Setting region to: ", newRegion);
    }
  
    // Оновлюємо мову та регіон
  ...
  // Використання useEffect для відслідковування змін
  useEffect(() => {
    const storedCountry = localStorage.getItem('selectedCountry');
    if (storedCountry) {
      handleCountryChange(storedCountry); // Автоматично оновлюємо мову і регіон при зміні країни
    }
  }, [country]);
